#!/usr/bin/env python
"""
OOT Benchmark Evaluation via local vLLM (OpenAI-compatible API).

Usage:
  python scripts/oot_benchmark/eval_local_vllm.py --model hydrogen-qwen-rl --port 8100
  python scripts/oot_benchmark/eval_local_vllm.py --model hydrogen-qwen-sft --port 8101
"""
import json
import os
import sys
...
# ─── Local vLLM ──────────────────────────────────────────────────────────────
def call_vllm(question, base_url, model, temperature=0.1, max_retries=3):
...
# ─── Main ─────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="OOT Benchmark Eval via local vLLM")
    parser.add_argument("--model", default="hydrogen-qwen-rl", help="vLLM served model name")
    parser.add_argument("--port", type=int, default=8100, help="vLLM server port")
    ...
    print(f"Model: {args.model} (local vLLM @ port {args.port})")
    ...
    for i, q in enumerate(questions):
        ...
        raw_response, in_tok, out_tok = call_vllm(question, base_url, args.model)

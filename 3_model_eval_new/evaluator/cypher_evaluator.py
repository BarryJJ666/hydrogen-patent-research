...
class CypherEvalResult:
    ...
    is_executable: bool
    is_result_match: bool       # 严格匹配（EX）
    is_semantic_match: bool     # 语义匹配（召回率≥80%）
...
class CypherEvaluator:
    ...
    def evaluate_single(self, question: str, gold_cypher: str,
        ...
        return CypherEvalResult(
            ...
            is_semantic_match=detail.get("semantic_match", False),
    ...
    def evaluate_batch(self, test_data: List[Dict],
        ...
        metrics.semantic_accuracy = sum(1 for r in results if r.is_semantic_match) / total if total > 0 else 0

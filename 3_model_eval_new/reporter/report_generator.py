...
class ReportGenerator:
    ...
    def __init__(self, run_output_dir: Path = None):
        """
        初始化报告生成器

        Args:
            run_output_dir: 运行输出目录（如果使用新的组织方式）
        """
    ...
    def generate_comparison_report(self, all_results: Dict, test_size: int) -> str:
        ...
        lines = [
            ...
            "## 评估结果对比",
            "",
            "| 模型 | 模式 | 准确率(EX) | 平均轮数 | 平均延迟(ms) | 平均输出Token |",
            "|------|------|------------|----------|-------------|---------------|",
        ]
        ...
        if self.run_output_dir:
            report_path = self.run_output_dir / "comparison.md"
        else:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    def generate_comparison_csv(self, all_results: Dict) -> str:
        """生成对比表格（CSV格式）"""
        csv_lines = [
            "model,mode,execution_accuracy,avg_turns,psjs,executable_rate,avg_latency_ms,avg_output_tokens"
        ]
        ...
        # 如果使用新的组织方式，保存到运行目录
        if self.run_output_dir:
            csv_path = self.run_output_dir / "comparison.csv"
        else:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
